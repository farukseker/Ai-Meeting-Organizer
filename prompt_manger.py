import os.path
from config import PROMPT_TEMPLATES_BASE_DIR
    ...
    def do_prompt_context(self, name: str, context: str) -> None:
        ...
        for context_var in context.split('\n'):
            if len(var := context_var.split(':')) == 2 and context_var.startswith('!'):
                ...
                prompt += ' {0}'.format(context_var)

        __prompt = Prompt(name=name, prompt=prompt, input_variables=input_variables, model=model)
...
if __name__ == '__main__':
    ...
    llm_ref = ChatGoogleGenerativeAI(model='gemini-2.0-flash', google_api_key=config.GEMINI_API_KEY)

    prompts = PromptManager(llm_ref)
